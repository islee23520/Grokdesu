...
Live Senpi (`app-server --listen stdio://`) and Grok adapters are implemented. Senpi waits for the matching terminal `turn/completed` notification before reading a transcript. Grok's current QA authentication blocker is environment-specific. Deterministic fixture sessions remain available only when `OMONATIVE_FIXTURES=1`; production startup uses live adapters.
...
Use an isolated `SENPI_CODING_AGENT_SESSION_DIR` beneath `.omo/qa-homes` for Senpi runtime QA while preserving the normal authenticated coding-agent directory. Do not write or inspect provider transcript/session files. Evidence is in `.omo/evidence/omonative`.
